...
function Navbar() {
  ...
  return (
    <nav className="fixed mt-0 w-full top-0 flex items-center justify-between flex-wrap bg-red-500 p-4 z-50">
      ...
      {/* Menu */}
      <div
        className="hidden md:flex items-center justify-end w-auto"
        id="menu"
      >
      ...
      <div onClick={handleNav} className="block px-4 cursor-pointer lg:hidden" id="burger">
        <svg
          className="w-6"
          fill="none"
          strokeLinecap="round"
      ...
      {/* Mobile menu */}
      <div
        className={!navBar ? "hidden" : "w-full items-center justify-end md:hidden"}
        id="menu"
      >
        {/* When navBar is false, nav is hidden and when navBar is true, nav is hidden on medium screen */}
        <div className="text-lg lg:flex-grow lg:mr-12 lg:text-sm sm:flex-col">
          <a
            href="#"
            className="mt-4 lg:mt-0 text-white hover:text-white mr-4"
          >
            <svg
              className="inline mb-1 w-5"
              fill="none"
